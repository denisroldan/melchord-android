...
public class DataManager {
    ...
    * It extensively uses RxJava operators to combine, filter and transform
    * data retrieved from helper classes.
    * The aim of the DataManager is to reduce the amount of work that Activities
    * and Fragments have to do by providing data that is ready to display and
    * won’t usually need any transformation.
    *
    * */
    ...
    @Inject
    public DataManager(ArtistService artistSearchService, ReleaseService releaseService,
                       SongService songService, TabService tabService,
    ...
    public Observable<List<ArtistSearch>> getSearchResults(final String query) {
        return mArtistService.getArtistSearchResults(query)
                .concatMap(new Func1<List<ArtistSearch>,
                        Observable<? extends List<ArtistSearch>>>() {
                    @Override
                    public Observable<? extends List<ArtistSearch>>
                            call(List<ArtistSearch> artistSearches) {
                                return mArtistService.getArtistSearchResults(query);
                            }
                });
    }
